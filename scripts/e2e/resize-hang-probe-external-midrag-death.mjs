...
// Env: E2E_BASE_URL, SESSION, CDP_PORT (default 9223), KILL_AFTER (default 5),
// DRAG_SECONDS (default 14).
...
for (let i = 0; i < steps; i++) {
  ...
  if (!killed && (Date.now() - t0) >= KILL_AFTER * 1000) {
    killed = true;
    // Kill the daemon serving this session (short scratch path, safe target).
    const pids = execSync(
      `ps aux | grep -E "herdr server" | grep -v grep | awk '{print $2}'`,
    ).toString().trim().split('\\n').filter(Boolean);
    console.log(`KILLING daemon pids at t+${KILL_AFTER}s:`, pids.join(','));
    for (const pid of pids) {
      try { process.kill(Number(pid), 'SIGTERM'); } catch (e) { console.log('kill failed:', e.message); }
    }
    // Mark the storm window start.
    await evalx(`(() => { globalThis.__probe.stormStart = Date.now(); return true; })()`);
    progress(`daemon killed at t+${KILL_AFTER}s`);
  }
