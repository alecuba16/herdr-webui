#!/usr/bin/env bash
# End-to-end acceptance run for live terminal resize (no WS reconnect).
#
...
# Chrome over CDP: resizing the browser window must send a live resize
# message on the open terminal socket (pty resizes in place, `stty size`
# matches the grid) instead of tearing down and re-attaching (which replays
# scrollback and flickers).
#
# Usage:
#   scripts/e2e/run-live-resize-e2e.sh [--keep]     # --keep skips teardown
#
...
PORT="${E2E_PORT:-8899}"
CDP="${CDP_PORT:-9222}"
WORK="$(mktemp -d "${TMPDIR:-/tmp}/herdr-live-resize-e2e.XXXXXX")"
KEEP=0
[[ "${1:-}" == "--keep" ]] && KEEP=1

session_id() {
  # Unique per run so two concurrent runs never share server state.
  printf 'live-resize-e2e-%s-%s' "$$" "$(date +%s)"
}
...
echo "==> running live resize acceptance checks"
ACCEPT_REPO="$REPO" E2E_BASE_URL="https://127.0.0.1:$PORT/" CDP_PORT="$CDP" \
  node "$ROOT/scripts/e2e/live-resize-acceptance.mjs"
