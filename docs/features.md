...
- Desktop and mobile use the header magnifier and the WebUI search shortcut as the single search entry point. The palette can show sections for workspaces/worktrees/panels, file names, folder names, and file contents for the focused workspace/worktree. Section enablement and ordering are configurable in Settings.
- File/folder sections call `/api/file-browser/tree?q=&offset=&limit=` in the backend, lazy-load pages, highlight matches, show parent folder context as an expanded tree, and preserve the file tree path instead of flattening results. File/folder backend search is configurable with `fileBrowserPathSearch` and `fileBrowserSearchPageSize`. `Alt+F` switches the active path section to files and `Alt+D` switches it to folders.
- Content search runs only after the configured minimum characters, scans in the backend, groups results by file, supports expand/collapse all, lazy-loads additional file groups and full per-file matches, shows colored match text plus colored matched-line context, and uses Git-diff-style arrow controls to request more context above or below. Open here opens the file in the file explorer at the matched line with editor highlight. Each match can also enable an editor-style snippet for safe hash-guarded save where editing is available.
