...
- Log supports single-commit compare/reset/rebase and shift-click two-commit compare. Selecting one commit loads a `Committed files` side preview by comparing that commit with its parent; picking a file opens the read-only commit-vs-parent diff. File history can show a temporary read-only commit diff or jump to the log commit.
