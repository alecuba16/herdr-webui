...
- The Files screen lists the workspace tree with lazy directory expansion, `h`/`u` go to the parent, `l` enters, and `/` filters entries. `Enter` expands directories or opens the file preview. `e` edits the open preview (Ctrl-S saves, Esc stops editing); prefix `e` edits the file highlighted in Git Changes. `R` renames the selected file (prefilled prompt) and `x` deletes it after a typed `y` confirmation.
