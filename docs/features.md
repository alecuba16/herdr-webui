...
- The Files screen lists the workspace tree with lazy directory expansion, `h`/`u` go to the parent, `l` enters, and `/` filters entries. `Enter` expands directories or opens the file preview. `R` renames the selected file (prefilled prompt) and `x` deletes it after a typed `y` confirmation.
- The Git screen has gitui-inspired tabs (`Tab` cycles Changes/Log/Branches/Stash): Changes shows status plus per-file diff with `+`/`-` coloring, `s` stages, `d` discards, `f` fetches, `p` pulls, `P` pushes, and `c`/`a` open the commit/amend message modal. Branches switch with `Enter` and delete with `D`; Stash lists entries, `Enter` applies, and `D` drops after a typed `y` confirmation. Prefix then `G` stages all changes.
