...
- The built-in backend starts with zero workspaces. Opening WebUI does not create a shell, workspace, tab, or pane until the user explicitly opens a workspace/worktree, starts an agent, or opens a temporary terminal.
- Session manager discovery scans external Herdr sockets under the Herdr config session directory and built-in socket namespaces under the WebUI config directory. Discovery is passive and never runs the Herdr binary. The UI can create a new built-in session inside the WebUI process or launch an external Herdr session through `HERDR_WEB_HERDR_BIN`/`herdr` only after an explicit external create/launch action. Settings can hide/disable either backend type, but at least one backend type must stay enabled.
...
- Desktop temporary terminals are fully keyboard-manageable: `Ctrl+B` then `Shift+M` opens, minimizes to the corner restore pill, or restores the same live temporary terminal. While the overlay is visible, Escape, Tab, and Backspace stay captured by the terminal, and `Ctrl+G` detaches it through the close confirmation. Temporary terminals start in the current, single, or visible workspace/worktree when possible, and only create a temporary workspace from the configured default folder when no workspace is available. Promoting (the `⤴` button or `Ctrl+B` then `Shift+P`) re-parents the live tab into a workspace rooted at the shell's current directory without stopping the process: the overlay closes, the shell keeps running in the new workspace, and the promoted folder lands in recent workspaces.
- Browser terminal input filters terminal-emulator query replies such as OSC 10/11 color responses before they reach the PTY. Native terminals consume those replies internally; the browser terminal renderer exposes them through `onData`, so WebUI drops them to avoid leaking `10;rgb:...`/`11;rgb:...` text into shells or agents.
...
- Agent identity detection covers Herdr-style direct argv aliases, wrapped shells, and terminal child process trees for Amp, Antigravity, Claude, Claurst, Cline, Codex, Cursor, Devin, Droid, Gemini, GitHub Copilot, Grok, Hermes, Jcode, Kilo, Kimi, Kiro, Maki, OpenCode, Pi, Qoder CLI, Qwen, plus legacy OMP and Mastra labels.
- Agent status detection falls back to terminal screen text for the built-in backend. It mirrors Herdr screen-manifest visible `blocked`, `working`, and `idle` rules where terminal text exposes the same controls for Amp, Antigravity, Claude, Claurst, Cline, Codex, Cursor, Devin, Droid, Gemini, GitHub Copilot, Grok, Hermes, Jcode, Kilo, Kimi, Kiro, Maki, OpenCode, Pi, Qoder CLI, and Qwen. Jcode status follows the Herdr `jcode-support` manifest rules and active background-task markers so status does not stay `working` once the prompt returns, but remains `working` while background tasks are still running. Jcode also emits OSC 9 structured payloads (`jcode:working`, `jcode:blocked`, `jcode:idle`) that the built-in backend captures as a preferred side-channel: fresh payloads override screen-scrape, stale payloads (older than 30s) fall back to screen-scrape, the payload prefix identifies jcode even through wrapper scripts, and terminal exit clears the tracker and reports `done` so status does not freeze after the process dies.
...
| Prefix then `Shift+P` | Temporary terminal overlay | Promotes the live temporary terminal into a workspace rooted at the shell's current directory; the process keeps running and the tab stays open. |
| `⤴` button | Temporary terminal overlay | Same promote as the shortcut: re-parents the tab into a workspace at the live cwd and navigates there. |
...
- The TUI mirrors the WebUI prefix-key pattern. `Ctrl+B` arms the prefix (the footer shows `Ctrl+B>`), the next key runs the matching WebUI default shortcut, `Esc` cancels, and pressing `Ctrl+B` again toggles prefix mode off. The prefix works in every screen, including while attached to a pane.
- Screens match the WebUI feature set. Prefix then `f` opens the Files explorer, `g` opens Git management, and `t` returns to the Terminal. Prefix then `1`-`4` jump to the Git Changes/Commit/Log/Stash views, and `?` opens the shortcut help overlay.
- The Files screen lists the workspace tree with lazy directory expansion, `h`/`u` go to the parent, `l` enters, and `/` filters entries. `Enter` expands directories or opens the file preview. `e` edits the open preview: typing inserts at the cursor, `Left`/`Right`/`Home`/`End` move it, `Ctrl-S` saves (the server rejects a save when the file changed on disk, 409), `Ctrl-R` reloads from disk discarding the buffer, and `Esc` stops editing. Unsaved edits survive screen switches like a dirty webui editor tab, and opening a different file while dirty is refused until the buffer is saved or reloaded. Prefix `e` edits the file highlighted in Git Changes. `R` renames the selected file (prefilled prompt) and `x` deletes it after a typed `y` confirmation.
- The Git screen has gitui-inspired tabs (`Tab` cycles Changes/Log/Branches/Stash/History): Changes shows status plus per-file diff with `+`/`-` coloring, `s` stages, `d` discards, `f` fetches, `p` pulls, `P` pushes, and `c`/`a` open the commit/amend message modal. Prefix then `h` shows the commits touching the file selected in Changes (webui file history) with `Enter` loading the selected commit's diff for that file, and `o` returns to the Changes view (webui compare). Prefix then `m` toggles blame on the Changes diff: each line is prefixed with the author git blames for it (webui `blame: KeyM`; `--contents` blame, so uncommitted lines show the synthetic `External file (--contents)` author), cached per file. Prefix then `y`/`u` always stage/unstage the selected file (matching the webui actions), and `G` toggles stage-all: it unstages everything when something is staged, otherwise stages all changes. Prefix then `0` opens the shortcut help overlay (webui `help: Digit0`); the webui `focusFile` (`I`) needs no TUI key because selection always lives in the file list. Branches switch with `Enter` and delete with `D`; Stash lists entries, `Enter` applies, and `D` drops after a typed `y` confirmation. Editing from Git (prefix `e`) jumps to the Files screen with the file loaded, mirroring how the WebUI edit button opens the file editor.
- File explorer and Git panels talk to the WebUI HTTP API over loopback. The TUI discovers the API base from `HERDR_WEBUI_TUI_API` (`HOST:PORT` or `http://HOST:PORT`), then the WebUI settings bind address, and finally `127.0.0.1:8787`. Pass `--webui-api HOST:PORT` to pin it explicitly. These screens require the WebUI server to be running; without it they show the API error in the status line.
...
- Remaining TUI gaps versus the WebUI: layout mutation, copy/search scrollback, mouse/touch, worktree dialogs, configurable keymaps, and notification integrations.
...
- Desktop uses authenticated `/api/file-browser/*` routes for tree, preview, edit/save with hash guard, rename, delete, and contextual open/split/copy actions. Mobile has browse/preview/edit parity: the same routes back mobile tree, preview, editing with hash-guarded save, rename, delete, and new-file actions, without split panes or editor find/replace.
...
- The directory picker starts from the configured default folder when the target input is empty or still contains the old root placeholder `/`. This applies to workspace/worktree opening and Git directory selection. The configured default folder resolves in order: the browser `Exploration default directory` option, then the server `default_folder` setting, then home (`~`). The picker's `Default dir` button follows the same resolution.
...
- Text files open editable by default; a lock toggle in the toolbar switches the focused file to read-only and back, tabs show a dirty dot for unsaved changes, and `Cmd/Ctrl+S` saves the focused file. Text previews include an editor find bar with next/previous, match-case, and regex search. Editable files enable replacement controls for one match or all matches; read-only (locked) files keep replace disabled.
- Markdown files (`.md`, `.markdown`) open as a rendered preview by default, with prose styling, tables, code blocks, and Mermaid diagrams rendered as SVG. The file browser toolbar shows an eye toggle for markdown files (styled like the editor find button) that flips between the rendered preview and the CodeMirror source view with find (editable when unlocked); the file context menu adds a `Preview` entry and the tab context menu adds `Preview`/`Show markdown source`. Mermaid is lazy-loaded only when a markdown file contains diagrams. Editing a markdown file keeps the CodeMirror editable view.
...
- Desktop has an embedded Git drawer backed by Rust API routes and the system `git` CLI; no Node/React/Vite runtime is needed. Mobile has grouped Git status with per-file stage/unstage/discard, branch list and switch, and read-only file diffs; commit/log/stash/cleanup/blame/history stay desktop-only.
...
- Cleanup uses safe delete first: `git branch -d` or `git worktree remove`. It retries force only when Git says force is required. The current `main`/`master` branch is protected, but the primary repository's current non-`main`/`master` branch can be selected; the backend checks out `main` or `master` before deleting that branch. Cleanup rows show whether each branch/worktree branch was pushed before, was not pushed, or has unknown push status. Primary/main worktrees are hidden from cleanup results.
...
- Diffs include collapse/show-all, change grouping, inline word highlights, context expansion, hunk stage/unstage/restore, blame, syntax highlighting for common languages, and in-diff hunk editing with hash guard saves. The `Edit` button opens the hunk editor in the current diff layout: side-by-side shows previous (read-only) and current (editable) panes, unified shows a single editable hunk column.
...
- Log supports single-commit compare/reset/rebase and shift-click two-commit compare. Selecting one commit loads a `Committed files` side preview by comparing that commit with its parent; picking a file opens the read-only commit-vs-parent diff. File history rows offer `View change` (the read-only commit-vs-parent diff for that file; Back returns to the history) and `Find in log` (the log scoped to that file, keeping the scope until cleared with the `×` in the location bar).
- Git drawer navigation uses a single always-visible location bar with state-derived breadcrumbs (`Changes`, file, `Committed <hash>`, scoped log). One `Back` action pops the internal view stack and never leaves the drawer; `Esc` steps back one view and only hides the drawer at the changes root. `View change`, `Find in log`, and `openFileHistory` reuse the existing workspace Git view instead of switching tools.
- Stash view shows a Zed/IntelliJ-style split layout: the side panel lists stashes (name, date, message) with clickable selection and a file tree for the selected stash, and the main panel renders the full side-by-side or unified diff of the selected stash (or a single file within it). Apply, Pop, and Drop actions are available for the selected stash. The stash diff includes untracked files via `git stash show -p --include-untracked`.
- Git shortcuts share the WebUI prefix (`Ctrl+B` by default) and have collision-checked recording. Main defaults: `1` changes, `2`/`C` commit, `3`/`L` log, `4` stash, `R` refresh, `G` stage/unstage all, `Y/U/D/Z` selected file actions, `H/M/E/O/V/I/0` history/blame/edit/compare/Git directory or branch dialog/focus/help.
...
- The `Recent workspaces` section lists the 8 most recently opened workspaces and worktrees, server-persisted in `webui-settings.json` (max 20 entries, pruned of duplicates). Entries whose folder no longer exists on disk are filtered out of the list and dropped from the saved list on load. Entries already open in this session stay visible but are grayed out and disabled so you cannot reopen them. Clicking a recent entry opens that specific workspace, not the keyboard-selected row. `Clear` empties the list server-side. Opening a recent entry reopens the workspace or worktree at its recorded path and lands directly on its first tab and terminal panel (the backend returns the focused tab and root pane alongside the workspace), with a 10s client cache to keep palette opens fast. Mobile surfaces the same list in the Worktrees screen with per-entry open/remove and a clear button; desktop manages entries in the search palette.
