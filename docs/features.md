...
- Terminal, Git, and Files controls behave like workspace-scoped shell tabs. Git and Files state follows the selected workspace/worktree, can be minimized/restored, and resumes the last section, selected file, open file tabs, split state, and navigation state when returning to that environment. Closing the workspace/worktree forgets that cached state. The temporary terminal stays global and independent.
...
- Files open as file explorer tabs. Selecting a file from the tree or a search result opens it inside the file explorer; Shift-click or context `Open in split` opens another side-by-side pane. Dirty edited files ask before closing. The file explorer keeps an in-memory state per open workspace/worktree, including selected file, search selections, split panes, open file tabs, and unsaved edit drafts while switching panels. Closing that workspace/worktree forgets the cached state.
...
- The directory picker used by Browse buttons keeps the shared polished tree UX: debounced filtering, current-folder row, styled tree entries, and an always-visible `...` go-up entry. Go-up transitions from home (`~`) to filesystem root (`/`) when at the top of the home directory.
...
- Text previews include an editor find bar with next/previous, match-case, and regex search. Edit mode enables replacement controls for one match or all matches; read-only preview keeps replace disabled. The find bar opens from the editor magnifier, the file pane magnifier, or `Cmd+F`/`Ctrl+F` when the editor or file explorer has focus. The file pane header shows the full path in its selector button, truncating with an ellipsis at the start when the path does not fit while keeping the full path as the tooltip.
...
- Non-Git folders are treated as plain folders in the workspace/worktree opener. The UI does not call Git branch APIs for them, avoiding noisy branch-load errors while keeping branch options populated for real Git repositories.
...
- Conflicts expose `Use HEAD`, `Use parent`, `Use remote`, and `Mark resolved` per file, plus rebase continue, skip, and abort controls when resolving a rebase.
...
- Open search from the top-right `⌕` button, with `Cmd+F`/`Ctrl+F` from the app shell, or with the keyboard prefix then `/`. When a file editor or file explorer pane has focus, `Cmd+F`/`Ctrl+F` opens the editor find/replace instead of the global app search.
...
- `Cmd+F`/`Ctrl+F` is reserved for search UX: global app search in the shell, editor find when an editor or file browser pane owns focus, and terminal-safe behavior for terminal `Ctrl+F`.
