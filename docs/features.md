...
- The Git screen has gitui-inspired tabs (`Tab` cycles Changes/Log/Branches/Stash): Changes shows status plus per-file diff with `+`/`-` coloring, `s` stages, `d` discards, `f` fetches, `p` pulls, `P` pushes, and `c`/`a` open the commit/amend message modal. Branches switch with `Enter` and delete with `D`; Stash lists entries, `Enter` applies, and `D` drops after a typed `y` confirmation. Prefix then `G` stages all changes. Editing from Git (prefix `e`) jumps to the Files screen with the file loaded, mirroring how the WebUI edit button opens the file editor.
