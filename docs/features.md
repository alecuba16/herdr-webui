...
- Conflicts expose `Use HEAD`, `Use parent`, `Use remote`, and `Mark resolved` per file in both the Conflicts view and conflicted file headers in unified or side-by-side Changes diffs, plus rebase continue, skip, and abort controls when resolving a rebase.
