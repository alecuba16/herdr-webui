...
- Conflicts expose `Use HEAD`, `Use parent`, `Use remote`, and `Mark resolved` per file in both the Conflicts view and conflicted file headers in unified or side-by-side Changes diffs. The side-by-side hunk editor also detects conflict marker blocks and shows per-block `Use HEAD`, `Use parent`, and `Use remote` controls when editing conflicted file content, plus rebase continue, skip, and abort controls when resolving a rebase.
