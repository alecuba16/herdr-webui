...
- Markdown files (`.md`, `.markdown`) open as a rendered preview by default, with prose styling, tables, code blocks, and Mermaid diagrams rendered as SVG. The file browser toolbar exposes `Preview`/`Source` toggles for markdown files; `Source` returns to the CodeMirror source view with find (editable when unlocked). Mermaid is lazy-loaded only when a markdown file contains diagrams. Editing a markdown file keeps the CodeMirror editable view.
