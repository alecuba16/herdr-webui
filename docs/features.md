...
- Core views cover status, commit, commit & push, pull, push, log, stash, cleanup, file history, conflicts, blame, hunk editing, side-by-side diffs, branch switching, and worktree actions. Push starts regular and exposes force-with-lease/force retry only after Git rejects it. The Commit action is shown only when the status contains staged, unstaged, untracked, or conflicted files.
- File lists are collapsible trees with optional filename-only mode, line counts, filtering, yellow match highlighting, and stable scroll while filtering/expanding. The file filter sits below the Git action toolbar so the exclusive view selector and actions stay visually grouped. Section headers expose add all/remove all pills before their counters for bulk stage/unstage, while right-click actions support stage/unstage/discard/stash with confirmations.
- Changes, log, stash, and cleanup use a segmented exclusive toggle like the workspace shell-mode controls because only one Git view is shown at a time. Log rows show decorated branch, remote, and tag refs as horizontal shadow labels beside the graph/message row, so commits stay readable while branch context stays visible.
...
- Git log returns structured commit data (`hash`, `author`, `date`, `refs`, `message`) alongside graph lines, with null-byte separators handled server-side so desktop graph rendering stays clean.
- Pull, push, and rebase actions use modal flows with branch selectors. Push starts as a regular push; if Git rejects it, the same Push modal reopens with force-with-lease and force retry options. Rebase can pull the selected branch first, and commit view can commit then push with the same failed-push retry flow. The Git refresh action is a theme-aware spinning icon beside the Git title. File view includes an inline Unified/Side-by-side toggle, and unified diffs use the same intra-line word highlighting as side-by-side diffs.
...
- Git shortcuts share the WebUI prefix (`Ctrl+B` by default) and have collision-checked recording. Main defaults: `1` changes, `2`/`C` commit, `3`/`L` log, `4` stash, `R` refresh, `Y/U/D/Z` selected file actions, and `H/M/E/O/V/I/0` history/blame/edit/compare/branch/focus/help.
