...
- Log rows show a hover card with branch/tag labels, full commit hash/title, exact commit date/time, and author. Description, date, and author filters narrow the rendered rows locally without re-fetching.
- The log starts with a bounded history page and shows a bottom `Load more changes` button when more commits are available. Each click increases the requested commit limit, keeps selected commits/filter state, and fetches another page from `/api/git-ui/log`.
