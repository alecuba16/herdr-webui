...
- Desktop and mobile Files use one search control exposed by the magnifier and by type-to-search focus. The scope pill cycles File, Folder, and Content. File/folder scopes call `/api/file-browser/tree?q=&offset=&limit=` in the backend, lazy-load pages, highlight matches, show parent folder context as an expanded tree, and preserve scroll/focus while results render.
- File/folder backend search is configurable with `fileBrowserPathSearch` and `fileBrowserSearchPageSize`. Files are the default scope; `Alt+F` switches to files, `Alt+D` switches to folders, and `Alt+C` switches to content. Folder search uses breadth-first traversal so nearby matching folders are returned before deep unrelated trees exhaust the search visit cap.
- File search is attached to the file list. Focusing the list and typing opens the same integrated search input; Backspace edits the query, Escape clears it, and the scope pill switches search type without creating separate controls.
- Content scope opens a closable right-side results tab on desktop and an inline results panel on mobile. It runs only after the configured minimum characters, scans in the backend, groups results by file, supports expand/collapse all, lazy-loads additional file groups and full per-file matches, shows highlighted line context, and has Open here for opening the file at the matched line with editor highlight. Each match can also enable an editor-style snippet for safe hash-guarded save.
...
- File tree indentation is configurable with `Tree indentation` and shared with Git file trees. Search defaults are configurable: file/folder search enablement, file/folder lazy page size, content-search minimum characters, content-search page size, context lines, auto-collapse threshold, and initial matches per file.
