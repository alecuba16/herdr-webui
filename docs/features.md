...
- Content search runs only after the configured minimum characters, scans in the backend, groups results by file, supports expand/collapse all, lazy-loads additional file groups and full per-file matches, shows highlighted line context, and has Open here for opening the file in the file explorer at the matched line with editor highlight. Each match can also enable an editor-style snippet for safe hash-guarded save where editing is available.
