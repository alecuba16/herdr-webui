...
- The Files screen lists the workspace tree with lazy directory expansion, `h`/`u` go to the parent, `l` enters, and `/` filters entries. `Enter` expands directories or opens the file preview.
- The Git screen has gitui-inspired tabs (`Tab` cycles Changes/Log/Branches/Stash): Changes shows status plus per-file diff with `+`/`-` coloring, `s` stages, `d` discards, `f` fetches, `p` pulls, `P` pushes, and `c`/`a` open the commit/amend message modal. Branches switch with `Enter`; Stash lists entries and `Enter` applies. Prefix then `G` stages all changes.
