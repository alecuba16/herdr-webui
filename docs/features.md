...
- The Git screen has gitui-inspired tabs (`Tab` cycles Changes/Log/Branches/Stash/History): Changes shows status plus per-file diff with `+`/`-` coloring, `s` stages, `d` discards, `f` fetches, `p` pulls, `P` pushes, and `c`/`a` open the commit/amend message modal. Prefix then `h` shows the commits touching the file selected in Changes (webui file history), and `o` returns to the Changes view (webui compare). Prefix then `y`/`u` always stage/unstage the selected file (matching the webui actions), and `G` toggles stage-all: it unstages everything when something is staged, otherwise stages all changes. Branches switch with `Enter` and delete with `D`; Stash lists entries, `Enter` applies, and `D` drops after a typed `y` confirmation. Editing from Git (prefix `e`) jumps to the Files screen with the file loaded, mirroring how the WebUI edit button opens the file editor.
