...
- Core views cover status, commit, commit & push, pull, push/force-push, log, stash, cleanup, file history, conflicts, blame, hunk editing, side-by-side diffs, branch switching, and worktree actions. The Commit action is shown only when the status contains staged, unstaged, untracked, or conflicted files.
...
- Changes, log, stash, and cleanup use a segmented exclusive toggle like the workspace shell-mode controls because only one Git view is shown at a time. Log rows show decorated branch, remote, and tag refs beside the commit graph in compact vertical pills.
