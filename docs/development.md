...
- `src/assets/shared/attention.js`: shared agent-attention helpers (`HerdrAttention`): agent sorting by attention rank/custom group order/working-first preset, and stuck-working dismissals persisted under the `herdr-web-working-dismissals` key shared by both layouts. `src/assets/mobile_parity.test.mjs` covers it.
...
- `src/assets/shared/markdown_preview.js`: shared markdown preview renderer (lazy marked, DOMPurify, stylesheet, and Mermaid).
- `src/assets/shared/markdown_preview.css`: shared markdown preview prose styling.
...
- `src/assets/vendor/marked_entry.mjs`, `dompurify_entry.mjs`, `mermaid_entry.mjs`: markdown preview bundle source entries; `scripts/build_markdown_assets.mjs` writes the checked-in `marked.bundle.js`, `dompurify.bundle.js`, and `mermaid.bundle.js` assets. Mermaid is lazy-loaded only when a markdown preview contains diagrams.
...
- Desktop and mobile UI are embedded vanilla HTML/CSS/JS assets. The main shell has no frontend build step; optional vendor bundles are checked in. CodeMirror is generated from `src/assets/vendor/codemirror_entry.mjs`; wterm/Ghostty assets are generated by `scripts/build_wterm_assets.mjs`; markdown preview assets (marked, DOMPurify, mermaid) are generated by `scripts/build_markdown_assets.mjs`.
...
- Desktop Git UI and desktop file browser controller code are lazy-loaded when those panels are opened. Shared helpers, color tokens, shared content-search CSS, file tree rendering, file icons, unified search helpers, content-search rendering, terminal fit/renderer/temp-terminal helpers, and the CodeMirror/editor shell load during boot. Markdown preview assets load only when a markdown preview opens, with Mermaid deferred until a diagram is present.
- The shared editor uses CodeMirror for file views (editable by default in the desktop file browser, with a lock toggle for read-only), with a lightweight numbered HTML fallback only if CodeMirror fails to load.
...
- Terminal renderer choice is optional in Settings. The default Ghostty core loads the embedded `/assets/vendor/ghostty-vt.wasm` asset and renders Kitty graphics inline; the wterm core is pure JS and shows an image placeholder. Browsers that saved options while wterm was the default migrate to Ghostty once unless the user re-chooses wterm. When changing renderer behavior, update desktop, mobile, temporary terminal, Help modal, and docs together.
...
- Both layouts support workspace selection, agent list/attention status, panel selection/creation/closing, linked worktree listing/creation/opening, terminal attach, bounded terminal paste, terminal scrollback follow/Tail behavior, terminal links, Files browsing with backend search/filter, file preview with editing and hash-guarded save, file rename/delete/new-file, Git status viewing with file filtering, per-file Git stage/unstage/discard, branch list and switch, read-only Git file diffs, recent workspaces with open/remove/clear, no-sleep control (off/auto/1h/2h/4h/infinite via the server), notification scope (current tab vs all tabs), agent sorting (attention rank, custom group order, working-first preset, shared with desktop through `shared/attention.js`), stuck-working dismiss with Undo and automatic clearing on status changes or terminal output, Settings, theme choice, browser notifications, local attention tone volume, file tree indentation, and layout preference.
...
- Mobile is intentionally narrower. It keeps navigation, agents, worktrees with recent workspaces, terminal, Files with editing/rename/delete/new-file, per-file Git stage/unstage/discard and branch switch with read-only diffs, and Settings (including no-sleep, notification scope, agent sorting, and stuck-working dismiss) usable on small screens, but does not yet expose desktop Git commit/log/stash/cleanup/blame/history, file split panes, editor find/replace, recent-workspace management inside the search palette, or server settings editing.
