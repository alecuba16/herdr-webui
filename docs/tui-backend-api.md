...
- `1`/`2`: Git Changes. `3`: Git Log. `4`: Git Stash. `b`: Git Branches. `h`: Git History for the file selected in Changes (`Enter` loads the selected commit's diff for that file, `o` back to Changes). `2`/`c` open the commit modal, `e` edits the currently selected file (the Files preview on the Files screen, the file highlighted in Git Changes otherwise), `G` toggles stage-all.
