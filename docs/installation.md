...
| `0.2.8` | `0.7.2` | `15` with `14` fallback | Superseded | Restores terminal paste compatibility while keeping the performance fix: paste avoids terminal renderer synchronous parsing and is sent as bounded WebSocket input chunks with backpressure. |
...
| `0.2.1` | `0.7.1` | `14` | Tested | Restores native terminal wheel scrolling by removing the custom shell wheel handler, enables terminal renderer viewport scrolling, uses dynamic terminal shell sizing without inline height/width, removes the custom terminal context menu, and deduplicates terminal CSS/JS into modular helpers. |
...
version over itself. Newer update commands print the running version
(`Updating herdr-web to v0.4.40`), warn when the running binary is already
the installed one, and report `Binary at ... is already <version>` / `TUI
binary at ... is already <version>` instead of a false update line, so a
no-op refresh is visible instead of silent.
