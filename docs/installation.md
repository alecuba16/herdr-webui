...
| --- | --- | --- | --- | --- |
| `0.2.98` | `0.8.0` | `20` with `18` and `16` fallback | Current | Advertises truecolor support to builtin-backend PTY children via `COLORTERM=truecolor`, so TUIs (k9s, etc.) emit SGR 38/48;2 RGB colors instead of quantized palette indexes that mapped to theme palette slots (k9s black bg rendered gray in dark theme). |
