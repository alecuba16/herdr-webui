...
| --- | --- | --- | --- | --- |
| `0.2.97` | `0.8.0` | `20` with `18` and `16` fallback | Current | Forwards alt-screen mouse wheel events as SGR mouse reports from the shared terminal adapter, enabling TUIs like jcode to scroll on the builtin backend. Removes the previous split `{type:"scroll"}` WebSocket protocol and Rust `sgr_scroll_reports` handler. |
