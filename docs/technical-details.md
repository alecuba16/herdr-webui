...
The Git sidebar separates exclusive view selection from action commands. The Git log endpoint asks `git log` for `%D` decorated refs and reconstructs display lines as `graph + short hash + refs + subject`, while also returning `refs` in the structured commit payload. The frontend splits those refs from the message and renders shortened branch/tag labels as low-opacity horizontal shadow text in the graph/message row, keeping commit subjects readable while preserving branch context. Changes, log, stash, and cleanup render as a segmented toggle styled like the workspace shell-mode controls. Worktree actions remain in the toolbar below that toggle, and the file filter is rendered under the action toolbar so filtering applies to the visible file lists without visually competing with view switching. Cleanup is text-only in the segmented toggle to keep the Git selector visually simple. Bulk stage/unstage is section-scoped: add all/remove all pills live inside each file-section header immediately before the counter, avoiding a global toolbar action with ambiguous scope. The Commit toolbar action is derived from the same status payload and is omitted when there is nothing committable. Push uses a single normal-push entry point; force-with-lease and force are only exposed if Git rejects the regular push, keeping destructive history overwrite out of the normal toolbar.
