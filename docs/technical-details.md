...
Disabled backends are never silently rerouted to. A request or WebSocket that targets a backend disabled in server settings is rejected with a clear `400 backend type is disabled in settings` error instead of falling back; routing helpers (`backend_target_for_*`) filter disabled targets and resolve to the server's configured default only when the request carries no explicit target. `/api/versions`, `/api/sessions`, and `/api/server-settings` responses include `enabled_backends` plus the server's configured `default_backend`, and every connected `/ws/events` socket receives a one-shot `server_settings_changed` broadcast (carrying the same two fields) whenever settings are saved, so open tabs retarget and stop offering a backend that was disabled mid-session without a page reload. Browser tabs validate their pinned backend against `enabled_backends` and cycle their events WebSocket after retargeting so the reconnect targets an enabled backend. The events socket itself stays open when its backend subscription fails (for example an external daemon that is down) and retries `events.subscribe` with a 1s-to-30s backoff, resetting after a stream stays healthy for 10s, because it also carries server-level frames such as `server_settings_changed` and LSP diagnostics that must not die with the backend.

### Built-in backend architecture

- `src/builtin_backend.rs` owns built-in runtime state: workspaces, tabs, panes, terminals, agent metadata, worktree helpers, local socket listeners, request dispatch, agent identity detection, and built-in screen status detection.
...
- The user-facing hierarchy is workspace/worktree -> panel. Internally, panels are represented as tabs, and each tab currently owns a root pane and terminal so future split/layout work can reuse the Herdr-shaped protocol. Display numbers are scoped by level: workspace numbers are unique across workspaces, tab/panel numbers restart inside each workspace, and pane numbers restart inside each tab. Entity IDs remain globally unique and opaque.
- Every opened workspace/worktree owns at least one panel with a live shell. This mirrors external Herdr, whose `worktree.open` and `workspace.create` always produce a tab with a root pane and whose close paths remove the workspace when the last pane closes; a panel-less workspace is not representable in either backend. Built-in `worktree.open` implements Herdr's already-open semantics: it matches the canonicalized checkout path against open workspaces, focuses the existing workspace and returns `already_open: true` when found, and creates a fresh workspace with one panel otherwise. A supplied `label` renames the already-open workspace, matching external Herdr.
...
1. **OSC 9 structured payloads** (preferred): Agents like jcode emit `ESC]9;jcode:working BEL` sequences. The `Osc9Tracker` state machine parses these in O(1) per byte with zero allocations after initial setup. OSC 9 payloads are treated as authoritative when fresh (within 30s of receipt), then fall back to screen-text when stale so a crashed or non-emitting agent does not leave a frozen status. The tracker also extracts the agent label from the payload prefix (`jcode:`), so jcode is identified even when argv and process-tree detection fail (wrapper scripts, shells). On terminal exit, the tracker is reset to prevent stale status from persisting after the process is gone.
...
- `/api/git-ui/log` asks Git for one more commit than the requested limit, returns `has_more` and `limit`, accepts an optional `file` path that is sanitized with `safe_repo_path`, and emits structured graph rows for the Zed-style Git log.
