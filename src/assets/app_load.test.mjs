...
describe("app bundle load", () => {
  ...
  it("keeps desktop terminal scrolling delegated to vanilla xterm", () => {
    ...
    match(desktopTerminalSource, /function sendBackendScrollNow\(lines\) \{[\s\S]*?type: "scroll"[\s\S]*?direction: wholeLines < 0 \? "up" : "down"/);
    match(desktopTerminalSource, /function sendBackendScroll\(lines, immediate = false\) \{[\s\S]*?terminalBackendScrollLines \+= wholeLines;[\s\S]*?requestAnimationFrame\(flushBackendScroll\);/);
    match(desktopTerminalSource, /function flushBackendScroll\(\) \{[\s\S]*?terminalBackendScrollLines = 0;[\s\S]*?sendBackendScrollNow\(lines\);/);
    ...
    match(source, /terminalBackendScrollLines = 0,[\s\S]*?terminalBackendScrollFramePending = false,/);
    match(source, /terminalBackendScrollLines = 0;[\s\S]*?terminalBackendScrollFramePending = false;[\s\S]*?if \(terminalWriteQueue\.length/);
    ...
    match(desktopTerminalSource, /function sendBackendTail\(\) \{[\s\S]*?terminalBackendScrollLines = 0;[\s\S]*?sendBackendScroll\(65535, true\)/);
