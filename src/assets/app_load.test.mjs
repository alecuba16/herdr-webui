...
describe("app bundle load", () => {
  ...
  it("defines file tree search and Git change filtering UI", () => {
    ...
    match(fileTreeSource, /entry\.kind === "more"/);
    match(fileTreeSource, /opts\.moreMethod \|\| "more"/);
    ...
    match(fileBrowserSource, /state\.search\.searching = true;\n\s+state\.search\.timer = setTimeout/);
    match(fileBrowserSource, /}, 500\);/);
    match(fileBrowserSource, /function restoreSearchInputFocus\(restore, start, end\)/);
    match(fileBrowserSource, /document\.activeElement === oldSearch/);
    match(fileBrowserSource, /input\.setSelectionRange\(start, end\)/);
    match(fileBrowserSource, /Change folder/);
    match(fileBrowserSource, /Workspace root/);
    match(fileBrowserSource, /async function changeFolder\(\)/);
    match(fileBrowserSource, /async function resetFolder\(\)/);
  ...
  it("defines Git file rename and staged edit menu actions", () => {
    ...
    match(gitUiSource, /Search branches/);
    match(gitUiSource, /function branchSection\(label, branches, kind\)/);
    match(gitUiSource, /function branchButton\(branch, kind\)/);
    match(gitUiSource, /filterBranchModal\(value\)/);
    match(gitUiSource, /switchBranch\(kind, encodedBranch\)/);
    ...
    match(fileBrowserSource, /function loadMoreChildren\(path\)/);
    match(fileBrowserSource, /state\.childHasMore\[path\] = !!data\.has_more/);
    match(fileBrowserSource, /kind: "more", name: state\.loading\[entry\.path\] \? "Loading\.\.\." : "\.\.\."/);
