...
describe("app bundle load", () => {
  ...
  it("renders shortcut editor with collision detection", () => {
    ...
    match(html, /Git selector opens repo tools for diff, stage\/unstage, discard, commit, commit & push, pull, push, rebase, conflicts, stash, branches, cleanup, and worktree prune/);
  ...
  it("defines Git cleanup tab and maintenance endpoints", () => {
    ...
    ok(!gitUiSource.includes("type === \"force-push\""));
    match(gitUiSource, /forceRetry/);
    match(gitUiSource, /Regular push failed/);
    match(gitUiSource, /Retry push/);
    match(gitUiSource, /renderGitViewTabs/);
    match(gitUiSource, /git-ui-view-toggle/);
    match(gitUiSource, /shortLogRefLabel/);
    match(gitUiSource, /git-ui-log-branch-shadow/);
