...
describe("app bundle load", () => {
  ...
  it("defines Git cleanup tab and maintenance endpoints", () => {
    ...
    match(cleanupRs, /pushed: b\.pushed/);
    match(cleanupRs, /pushed_by_branch/);
    match(branchRs, /fn maybe_switch_before_delete/);
    match(branchRs, /\["switch", "--", default_branch\]/);
  ...
  it("uses a log row context menu with reset and clear rebase wording", () => {
    ...
    match(readFileSync(new URL("./desktop/git_ui/diff_view.js", import.meta.url), "utf8"), /function historicalFileCommitLabel\(view\)/);
    ...
    match(readFileSync(new URL("./desktop/git_ui/diff_view.js", import.meta.url), "utf8"), /const committed = view && view\.committedFile;/);
    ...
    match(gitUiSource, /async goBack\(\) \{[\s\S]*?this\.showChangesList\(\);/);
    ok(!gitUiSource.includes("backToFileView"));
    ok(!gitUiSource.includes("backToFileHistory"));
    ok(!gitUiSource.includes("backFromFileView"));
    ok(!gitUiSource.includes("fileBackTarget"));
    ok(!gitUiSource.includes("historySource"));
    match(gitUiSource, /view\.tab = "log";/);
    match(gitUiSource, /workspaceKey\(\{ cwd \}\)/);
  ...
  it("renders stash view with diff preview and file tree", () => {
    ...
    // Title rendering
    match(readFileSync(new URL("./desktop/git_ui/workspace_nav.js", import.meta.url), "utf8"), /function viewCrumbs\(view\) \{[\s\S]*?if \(view\.tab === "stash"\) return view\.selectedStash \? \["Stash", view\.selectedStash\] : \["Stash"\];/);
