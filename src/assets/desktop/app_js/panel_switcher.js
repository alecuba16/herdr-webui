...
function panelActivityBadge(tab) {
  if (!options.showTabActivity || !tab) return "";
  const activity = tabActivity[tabActivityKey(tab.workspace_id, tab.tab_id)];
  const label = activity && tabActivityLabel(activity.updatedAt, Date.now());
  return label ? `<span class="tab-activity">${escapeHtml(label)}</span>` : "";
}

function panelMenuLabel(tab, index, tabs) {
  return `${escapeHtml(panelVisibleLabel(tab, index, tabs))}${panelActivityBadge(tab)}`;
}
...
function renderPanelField() {
  ...
  const currentActivity = panelActivityBadge(current);
