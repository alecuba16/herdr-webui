...
function renderPanelField() {
  ...
  if (current && state.editingTab === current.tab_id)
    return `<div class="panel-field"><input class="panel-label panel-rename-input tab-rename-input" value="${escapeAttr(state.editingTabValue)}" onmousedown="event.stopPropagation()" onclick="event.stopPropagation()" onblur="commitTabRename('${current.tab_id}')" oninput="state.editingTabValue=this.value" onkeydown="tabRenameKey(event,'${current.tab_id}')"><button class="mini panel-add" title="New panel" onclick="event.preventDefault();event.stopPropagation();newTab()">+</button>${state.supportsPaneReset && state.pane ? `<button class="mini panel-reset" title="Reset terminal modes (clear mouse tracking, alternate screen, etc.)" onclick="event.preventDefault();event.stopPropagation();resetTerminal()">⟳</button>` : ""}<button class="mini warn panel-close" title="Close current panel" onclick="event.preventDefault();event.stopPropagation();closeTab('${current.tab_id}')">✕</button></div>`;
  ...
  const reset = current && state.supportsPaneReset && state.pane ? `<button class="mini panel-reset" title="Reset terminal modes (clear mouse tracking, alternate screen, etc.)" onclick="event.preventDefault();event.stopPropagation();resetTerminal()">⟳</button>` : "";
  return `<div class="panel-field"><button class="panel-label" title="Current panel: ${escapeAttr(currentLabel)}. Double-click to rename." onclick="event.preventDefault();event.stopPropagation();${click}" ondblclick="event.preventDefault();event.stopPropagation();state.panelMenuOpen=false;${current ? `startTabRename('${current.tab_id}','${escapeAttr(currentLabel)}')` : ""}"><span>${escapeHtml(currentLabel)}</span>${caret}</button>${menu}<button class="mini panel-add" title="New panel" onclick="event.preventDefault();event.stopPropagation();newTab()">+</button>${reset}${close}</div>`;
}
