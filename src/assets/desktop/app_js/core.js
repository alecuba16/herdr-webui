...
function normalizeOptions(value) {
  ...
  next.worktreeDiscoverDepth = Math.max(
    0,
    Math.min(5, Math.floor(Number(next.worktreeDiscoverDepth) || 0)),
  );
...
function applyOptions() {
  ...
  const overflow = el("optOverflow"),
    ...
    worktreeDiscoverDepth = el("optWorktreeDiscoverDepth"),
  ...
  if (worktreeDiscoverDepth)
    worktreeDiscoverDepth.value = String(options.worktreeDiscoverDepth ?? 0);
