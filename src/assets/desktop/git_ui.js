(function () {
  ...
  function renderEditableHunkUnified(hunk) {
    ...
    const meta = hunk.newStart ? `current lines ${hunk.newStart}-${hunk.newEnd}` : "no current lines";
    const conflictControls = renderEditableHunkConflictControls(hunk);
    return `<div class="git-ui-hunk-editor git-ui-hunk-editor-unified"><div class="git-ui-hunk-head"><span>${esc(hunk.header || "hunk")}</span><span class="git-ui-muted">${esc(meta)}</span></div>${conflictControls}<div class="git-ui-hunk-editor-grid git-ui-hunk-editor-grid-unified"><section><div class="git-ui-editor-head"><strong>Editable hunk</strong></div><div class="git-ui-hunk-edit-mount git-ui-hunk-current-mount" data-hunk-index="${hunk.index}" data-editor-side="current" data-readonly="${hunk.newStart ? "false" : "true"}"></div><textarea class="git-ui-hunk-edit git-ui-hunk-current-hidden git-ui-hunk-edit-hidden" data-hunk-index="${hunk.index}" spellcheck="false"${readonly}>${esc(currentText)}</textarea></section></div></div>`;
  }
