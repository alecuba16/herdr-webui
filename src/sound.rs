...
pub fn play(sound: Sound, config: &crate::config::SoundConfig) {
    ...
    let volume_percent = config.volume;
    std::thread::spawn(move || {
        if let Some(path) = custom_path {
            match play_file(&path, volume_percent) {
                Ok(()) => return,
                Err(err) => {
        ...
        if let Err(err) = play_bytes(data, volume_percent) {
...
fn play_file(path: &Path, volume_percent: u8) -> Result<(), String> {
    match run_player(path, volume_percent) {
...
fn play_bytes(data: &[u8], volume_percent: u8) -> Result<(), String> {
    ...
    let result = run_player(&tmp, volume_percent);
...
fn volume_multiplier(volume_percent: u8) -> f32 {
    (volume_percent as f32 / 100.0).clamp(0.0, 2.0)
}

#[cfg(windows)]
fn run_player(path: &Path, volume_percent: u8) -> Result<Output, String> {
    run_windows_player(path, volume_percent)
}

#[cfg(target_os = "macos")]
fn run_player(path: &Path, volume_percent: u8) -> Result<Output, String> {
    Command::new("afplay")
        .args(["-v", &format!("{:.2}", volume_multiplier(volume_percent))])
        .arg(path)
        .output()
...
fn run_player(path: &Path, volume_percent: u8) -> Result<Output, String> {
    run_linux_player(path, volume_percent)
}
...
$player.Volume = [Math]::Min([Math]::Max($Volume, 0.0), 1.0)
$script:done = $false
$script:failed = $null
...
#[cfg(windows)]
fn run_windows_player(path: &Path, volume_percent: u8) -> Result<Output, String> {
    Command::new("powershell.exe")
        ...
        .arg(path)
        .arg(format!("{:.2}", volume_multiplier(volume_percent)))
        .output()
...
#[cfg(not(any(windows, target_os = "macos")))]
impl AudioPlayer {
    fn output(self, path: &Path, volume_percent: u8) -> std::io::Result<Output> {
        let volume = volume_multiplier(volume_percent);
        let mut command = Command::new(self.program);
        command.args(self.args);
        match self.program {
            "ffplay" => {
                command.args(["-af", &format!("volume={volume:.2}")]);
            }
            "mpg123" => {
                let scale = (32768.0 * volume).round() as u32;
                command.args(["-f", &scale.to_string()]);
            }
            "mpv" => {
                command.arg(format!("--volume={:.0}", volume * 100.0));
            }
            _ => {}
        }
...
#[cfg(not(any(windows, target_os = "macos")))]
fn run_linux_player(path: &Path, volume_percent: u8) -> Result<Output, String> {
    let mut errors = Vec::new();

    for player in linux_audio_players() {
        match player.output(path, volume_percent) {
...
mod tests {
    ...
    #[test]
    fn volume_multiplier_clamps_to_supported_range() {
        assert_eq!(volume_multiplier(0), 0.0);
        assert_eq!(volume_multiplier(100), 1.0);
        assert_eq!(volume_multiplier(200), 2.0);
        assert_eq!(volume_multiplier(255), 2.0);
    }
    ...
    fn windows_media_player_script_accepts_literal_path_argument() {
        ...
        assert!(script.contains("param([string]$Path, [double]$Volume)"));
        assert!(script.contains("Resolve-Path -LiteralPath $Path"));
        assert!(script.contains("System.Windows.Media.MediaPlayer"));
        assert!(script.contains("$player.Volume = [Math]::Min([Math]::Max($Volume, 0.0), 1.0)"));
    }
}
