...
async function createDiscoveredWorktree(){const err=el('worktreeOpenError'),sourcePath=el('worktreeDiscoverPath').value.trim(),branch=el('worktreeNewBranch').value.trim(),base=el('worktreeNewBase').value.trim(),label=el('worktreeNewLabel').value.trim(),path=el('worktreeNewPath').value.trim(),submit=el('worktreeNewSubmit');err.textContent='';const checkedOut=checkedOutWorktreeForBranch(branch);if(checkedOut){err.textContent=`Branch "${branch}" is already checked out at ${textValue(checkedOut.path)}`;return}if(branchExistsForNewWorktree(branch)){err.textContent=`Branch "${branch}" already exists`;return}submit.disabled=true;try{if(sourcePath&&!state.openWorktreeSource)await discoverWorktrees();const source=state.openWorktreeSource||{};const workspaceId=source.workspace_id||(!sourcePath?state.ws:null),cwd=source.workspace_id?null:(source.cwd||sourcePath||null);const r=await api('/api/worktrees',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({workspace_id:workspaceId,cwd,branch:branch||null,base:base||null,label:label||null,path:path||null})});closeWorktreeOpenModal();const result=r.result||{};go(result.workspace.workspace_id,result.tab&&result.tab.tab_id,result.root_pane&&result.root_pane.pane_id)}catch(ex){err.textContent=ex.message||String(ex)}finally{submit.disabled=false}}
