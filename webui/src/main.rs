...
const layoutStyle=document.createElement('style');layoutStyle.textContent='.side .section{flex:1;min-height:0;display:flex;flex-direction:column;overflow:hidden;padding:0}.sidebar-split{flex:1;min-height:0;display:flex;flex-direction:column}.sidebar-pane{min-height:0;display:flex;flex-direction:column;padding:0 10px 8px}.sidebar-pane.workspaces-pane{flex:1 1 auto;min-height:45%}.sidebar-pane.agents-pane{flex:0 1 32%;min-height:110px;margin-top:auto}.sidebar-scroll{min-height:0;overflow:auto;padding-right:2px}.side-footer{flex:none}.section-header{flex:none}.head #newWs,.head #openWorktrees{width:34px;height:34px;padding:0;display:grid;place-items:center}.head #openWorktrees{background:var(--accent);color:var(--accent-fg);border:1px solid var(--accent)}.tabs-tools{margin-left:auto;display:flex;gap:6px;align-items:center}.tabs-tools .mini{margin-left:0}.tabs .tab-info{margin-left:0}.head #sessionButton,.head #themeToggle,.head #shortcutsToggle,.head #settingsToggle{display:none}';document.head.appendChild(layoutStyle);
const accentStyle=document.createElement('style');accentStyle.textContent='body{--accent-fg:var(--bg);--accent-soft:color-mix(in srgb,var(--accent),transparent 84%);--accent-border:color-mix(in srgb,var(--accent),transparent 45%);--action-bg:var(--border);--action-fg:var(--fg);--action-border:var(--border2)}.btn,.tab.add,.sidebar-actions .btn,.sidebar-actions .worktree-open-trigger{background:var(--accent);color:var(--accent-fg);border:1px solid var(--accent)}.btn:hover,.tab.add:hover{background:color-mix(in srgb,var(--accent),white 10%)}.sidebar-actions .btn:hover,.sidebar-actions .btn:focus,.sidebar-actions .worktree-open-trigger:hover,.sidebar-actions .worktree-open-trigger:focus{background:#1e3a8a;color:#dbeafe;border-color:#1d4ed8}.tab.active{background:var(--accent);color:var(--accent-fg);border-color:var(--accent)}.item.active{background:var(--accent-soft);border-color:var(--accent-border)}.item.active:hover{background:color-mix(in srgb,var(--accent),transparent 78%);border-color:var(--accent)}.mini,.mini.danger,.mini.warn,.mini.tree,.tab.active .mini{background:var(--action-bg);color:var(--action-fg);border-color:var(--action-border)}.mini:hover,.mini:focus,.tab.active .mini:hover,.tab.active .mini:focus{background:var(--border2);color:var(--fg);border-color:var(--accent-border)}.mini.danger:hover,.mini.danger:focus,.tab.active .mini.danger:hover,.tab.active .mini.danger:focus{background:#5a2734;color:#ffccd5;border-color:#f38ba8}.mini.warn:hover,.mini.warn:focus,.tab.active .mini.warn:hover,.tab.active .mini.warn:focus{background:#56401b;color:#f9e2af;border-color:#f9e2af}.mini.tree:hover,.mini.tree:focus{background:#315044;color:#a6e3a1;border-color:#74c7a3}body.light .sidebar-actions .btn,body.light .sidebar-actions .worktree-open-trigger{background:var(--accent);color:var(--accent-fg);border-color:var(--accent)}body.light .sidebar-actions .btn:hover,body.light .sidebar-actions .btn:focus,body.light .sidebar-actions .worktree-open-trigger:hover,body.light .sidebar-actions .worktree-open-trigger:focus{background:#1e3a8a;color:#dbeafe;border-color:#1d4ed8}';document.head.appendChild(accentStyle);
...
const repoHeaderStyle=document.createElement('style');repoHeaderStyle.textContent='.repo-header.with-actions{justify-content:space-between}.repo-header.with-actions:after{display:none}.repo-header .repo-actions{margin-left:auto;display:inline-flex;gap:4px;align-items:center}.repo-header .mini{font-size:11px;padding:1px 5px;text-transform:none;letter-spacing:0}';document.head.appendChild(repoHeaderStyle);
const agentLayoutStyle=document.createElement('style');agentLayoutStyle.textContent='.agent-title{display:flex;align-items:center;gap:3px;min-width:0;width:100%}.agent-title .dot{flex:none}.agent-title .agent-token{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.agent-title .agent-repo,.agent-title .agent-panel{flex:0 0 auto;max-width:33%}.agent-title .agent-repo{font-weight:800;color:var(--fg)}.agent-title .agent-worktree{flex:1 1 auto;max-width:none}.agent-title .agent-sep{flex:none;color:var(--muted);font-size:10px}.agent-title .agent-panel{color:var(--muted)}';document.head.appendChild(agentLayoutStyle);
...
const chipThemeStyle=document.createElement('style');chipThemeStyle.textContent='body:not(.light) .chip.branch{border-color:#b4befe;color:#b4befe}body:not(.light) .chip.repo{border-color:#f9cb9c;color:#f9cb9c}body:not(.light) .chip.worktree{border-color:#a6e3a1;color:#a6e3a1}body.light .chip.branch{border-color:#174ea6;color:#174ea6}body.light .chip.repo{border-color:#9a3412;color:#9a3412}body.light .chip.worktree{border-color:#166534;color:#166534}';document.head.appendChild(chipThemeStyle);
const sectionEl=document.querySelector('.side .section');if(sectionEl&&!el('workspacePane')){const versionsEl=el('versions'),workspacesEl=el('workspaces'),agentsEl=el('agents'),oldAgentsHeader=document.querySelector('.section-header');const split=document.createElement('div');split.className='sidebar-split';const workspacePane=document.createElement('div');workspacePane.id='workspacePane';workspacePane.className='sidebar-pane workspaces-pane';workspacePane.insertAdjacentHTML('beforeend','<div class="section-header">Workspaces</div>');const workspaceScroll=document.createElement('div');workspaceScroll.className='sidebar-scroll';workspaceScroll.appendChild(workspacesEl);workspacePane.appendChild(workspaceScroll);const agentsPane=document.createElement('div');agentsPane.className='sidebar-pane agents-pane';agentsPane.innerHTML='<div class="section-header">Agents</div>';const agentsScroll=document.createElement('div');agentsScroll.className='sidebar-scroll';agentsScroll.appendChild(agentsEl);agentsPane.appendChild(agentsScroll);if(oldAgentsHeader)oldAgentsHeader.remove();split.appendChild(workspacePane);split.appendChild(agentsPane);sectionEl.appendChild(split);if(versionsEl){versionsEl.classList.add('side-footer');document.querySelector('.side').appendChild(versionsEl)}}
...
const worktreeStyle=document.createElement('style');worktreeStyle.textContent='.worktree-form{display:grid;gap:12px;padding:16px 20px}.worktree-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.worktree-form label{display:grid;gap:6px;color:var(--muted);font-size:12px}.worktree-form input{font-size:14px}.worktree-source{display:inline-flex;align-items:center;gap:8px;border:1px solid var(--border);background:var(--panel2);border-radius:999px;padding:6px 10px;color:var(--fg);width:max-content;max-width:100%}.worktree-error{min-height:18px;color:#f38ba8;font-size:13px;padding:0 20px}.mini.tree{font-weight:800}.worktree-open-controls{display:grid;gap:10px;padding:16px 20px 0}.worktree-open-controls label{display:grid;gap:6px;color:var(--muted);font-size:12px}.worktree-open-controls input{font-size:14px}.worktree-loading{display:none;align-items:center;gap:8px;color:var(--muted);font-size:12px}.worktree-loading.show{display:flex}.worktree-loading:before{content:"";width:10px;height:10px;border:2px solid var(--border2);border-top-color:#74c7a3;border-radius:50%;animation:spin .8s linear infinite}.worktree-open-list{display:grid;gap:8px;padding:12px 20px;max-height:min(340px,38vh);overflow:auto}.worktree-open-row{display:flex;align-items:center;justify-content:space-between;gap:12px;border:1px solid var(--border);background:var(--panel2);border-radius:12px;padding:10px}.worktree-open-row.selected{border-color:var(--accent);background:color-mix(in srgb,var(--accent),transparent 86%)}.worktree-open-row.disabled{opacity:.6}.worktree-open-row strong{display:block}.worktree-open-row small{display:block;color:var(--muted);margin-top:2px;word-break:break-all}.worktree-open-empty{padding:18px 20px;color:var(--muted)}.worktree-new{border-top:1px solid var(--border);background:color-mix(in srgb,var(--panel2),transparent 50%)}.worktree-new-head{padding:16px 20px 0}.worktree-new-head strong{display:block}.worktree-new-head small{display:block;color:var(--muted);margin-top:3px}.worktree-open-footer{display:flex;justify-content:flex-end;gap:8px;padding:0 20px 20px}';document.head.appendChild(worktreeStyle);
...
if(!el('worktreeOpenModal'))document.body.insertAdjacentHTML('beforeend','<div class="modal-backdrop" id="worktreeOpenModal"><div class="modal"><div class="settings-head"><div><h2>Worktrees</h2><p>Type a repo path to open existing linked worktrees or create a new one.</p></div><button class="mini settings-close" id="worktreeOpenClose" title="Close">✕</button></div><div class="worktree-open-controls"><label>Repo or worktrees folder<input id="worktreeDiscoverPath" list="worktreePathOptions" placeholder="~/Documents/code/repo-or-worktrees"></label><datalist id="worktreePathOptions"></datalist><div class="worktree-loading" id="worktreeLoading">Discovering worktrees...</div></div><div class="worktree-open-list" id="worktreeOpenList"></div><div class="worktree-new" id="worktreeNewSection"><div class="worktree-new-head"><strong>Create a new worktree</strong><small>Uses repo path above. Leave base blank to use repo default branch.</small></div><form class="worktree-form" id="worktreeNewForm"><div class="worktree-grid"><label>Branch name<input id="worktreeNewBranch" placeholder="feature/my-branch"></label><label>Base branch<input id="worktreeNewBase" list="worktreeBranchOptions" placeholder="default branch"></label><datalist id="worktreeBranchOptions"></datalist></div><div class="worktree-grid"><label>Label<input id="worktreeNewLabel" placeholder="optional"></label><label>Checkout path<input id="worktreeNewPath" placeholder="backend default if blank"></label></div><button class="btn" id="worktreeNewSubmit">New worktree</button></form></div><div class="worktree-error" id="worktreeOpenError"></div><div class="worktree-open-footer"><button type="button" class="tab add" id="worktreeOpenRefresh">Refresh</button></div></div></div>');
...
const themes={dark:{background:'#11111b',foreground:'#cdd6f4',cursor:'#1e66f5',cursorAccent:'#ffffff',selectionBackground:'#1e66f5aa',black:'#6c7086',brightBlack:'#9399b2',red:'#f38ba8',green:'#a6e3a1',yellow:'#f9e2af',blue:'#89b4fa',magenta:'#cba6f7',cyan:'#94e2d5',white:'#cdd6f4',brightWhite:'#ffffff'},light:{background:'#eff1f5',foreground:'#4c4f69',cursor:'#1e66f5',cursorAccent:'#ffffff',selectionBackground:'#1e66f599',black:'#5c5f77',brightBlack:'#6c6f85',red:'#d20f39',green:'#40a02b',yellow:'#df8e1d',blue:'#1e66f5',magenta:'#8839ef',cyan:'#179299',white:'#acb0be',brightWhite:'#4c4f69'}};function normalizeThemeMode(value){if(value==='night')return 'dark';if(value==='day')return 'light';return ['auto','light','dark'].includes(value)?value:'auto'}let themeMode=normalizeThemeMode(localStorage.getItem('herdr-web-theme')),lastEffectiveTheme=null;
let options=JSON.parse(localStorage.getItem('herdr-web-options')||'{"overflow":true,"fitToBrowser":false,"sound":true,"soundScope":"current","shiftEnterNewline":true,"closeShortcut":"off","sortAgentsByStatus":false,"workspaceSort":"default","scrollLines":3,"worktreeAutoDiscoverSeconds":3}');delete options.shiftEnter;localStorage.removeItem('herdr-web-shiftenter-migrated');if(options.shiftEnterNewline===undefined)options.shiftEnterNewline=true;if(options.captureCmdW===true||options.closeShortcut===true)options.closeShortcut='altw';if(!['off','altw','shiftspacew'].includes(options.closeShortcut))options.closeShortcut='off';if(options.sortAgentsByStatus===undefined)options.sortAgentsByStatus=false;if(!['all','current'].includes(options.soundScope))options.soundScope='current';if(!['default','drag','state'].includes(options.workspaceSort))options.workspaceSort='default';options.scrollLines=Math.max(1,Math.min(20,Number(options.scrollLines)||3));options.worktreeAutoDiscoverSeconds=Math.max(0,Math.min(30,Number.isFinite(Number(options.worktreeAutoDiscoverSeconds))?Number(options.worktreeAutoDiscoverSeconds):3));
function saveOptions(){localStorage.setItem('herdr-web-options',JSON.stringify(options))}
saveOptions();
const soundSetting=el('optSound');if(soundSetting&&!el('optSortAgents'))soundSetting.closest('label').insertAdjacentHTML('afterend','<label class="option"><span>Close panel shortcut<small>Stored in browser storage and available after reopening the tab.</small></span><select class="settings-select" id="optCloseShortcut"><option value="off">Disabled</option><option value="altw">Option+W</option><option value="shiftspacew">Shift+Space then W</option></select></label><label class="option"><input type="checkbox" id="optSortAgents"><span>Sort agents by attention<small>Blocked first, then done, unknown, idle, working.</small></span></label><label class="option"><span>Workspace sorting<small>Default tree order, shared drag-and-drop order, or attention state priority.</small></span><select class="settings-select" id="optWorkspaceSort"><option value="default">Default</option><option value="drag">Drag&drop</option><option value="state">State</option></select></label><label class="option"><span>Notification scope<small>Choose whether sounds ring in every open tab or only the tab viewing the agent panel.</small></span><select class="settings-select" id="optSoundScope"><option value="current">Current agent tab</option><option value="all">All tabs</option></select></label><label class="option"><span>Scroll speed<small><span id="scrollLinesValue">3</span> terminal lines per wheel step.</small></span><input type="range" id="optScrollLines" min="1" max="20" step="1"></label><label class="option"><span>Worktree autodiscover<small>Seconds to wait after path input stops. Set 0 for immediate.</small></span><input type="number" id="optWorktreeAutoDiscover" min="0" max="30" step="0.5"></label>');
function applyOptions(){const shell=el('terminalShell');if(shell)shell.classList.toggle('no-overflow',!options.overflow);const overflow=el('optOverflow'),fitOpt=el('optFit'),shiftEnterNewline=el('optShiftEnterNewline'),sound=el('optSound'),themeSelect=el('optTheme'),closeShortcut=el('optCloseShortcut'),sortAgents=el('optSortAgents'),workspaceSort=el('optWorkspaceSort'),soundScope=el('optSoundScope'),scrollLines=el('optScrollLines'),scrollLinesValue=el('scrollLinesValue'),worktreeAutoDiscover=el('optWorktreeAutoDiscover'),closeShortcutCurrent=el('closeShortcutCurrent');if(overflow)overflow.checked=!!options.overflow;if(fitOpt)fitOpt.checked=!!options.fitToBrowser;if(shiftEnterNewline)shiftEnterNewline.checked=options.shiftEnterNewline!==false;if(sound)sound.checked=!!options.sound;if(themeSelect)themeSelect.value=themeMode;if(closeShortcut)closeShortcut.value=options.closeShortcut||'off';if(closeShortcutCurrent)closeShortcutCurrent.textContent=closeShortcutLabel();if(sortAgents)sortAgents.checked=!!options.sortAgentsByStatus;if(workspaceSort)workspaceSort.value=options.workspaceSort||'default';if(soundScope)soundScope.value=options.soundScope||'current';if(scrollLines)scrollLines.value=String(options.scrollLines||3);if(scrollLinesValue)scrollLinesValue.textContent=String(options.scrollLines||3);if(worktreeAutoDiscover)worktreeAutoDiscover.value=String(options.worktreeAutoDiscoverSeconds??3);fitTerminalShell();if(options.fitToBrowser){const fit=browserTerminalSize();if(fit){state.termCols=fit.cols;state.termRows=fit.rows;connectTerminal()}}}
...
function saveCloseShortcutOption(){options.closeShortcut=el('optCloseShortcut').value;closeChordUntil=0;saveOptions();applyOptions()}
...
function apiErrorMessage(body,statusText){const err=body&&body.error;if(!err)return statusText;if(typeof err==='string')return err;if(err.message)return String(err.message);if(err.code)return String(err.code);try{return JSON.stringify(err)}catch(_){return statusText}}
async function api(url,opt){const r=await fetch(url,apiOptions(opt));if(r.status===401){location.href='/';throw Error('unauthorized')}const body=await r.json();if(!r.ok||body.error)throw Error(apiErrorMessage(body,r.statusText));return body}
...
function resetTerminalConnection(clear=false){if(inputFlushTimer){clearTimeout(inputFlushTimer);inputFlushTimer=null}inputQueue=[];if(termWs){termWs.onclose=null;try{termWs.close()}catch(e){}termWs=null}connectedTerminalId=null;connectedSize='';if(clear&&term)term.clear()}
...
if(seq!==refreshSeq)return;
state.worktrees=worktreeResults.flatMap(r=>{const result=((r||{}).result)||{},source=result.source||{};return (result.worktrees||[]).map(wt=>Object.assign({},wt,{source_workspace_id:source.source_workspace_id,source_repo_name:source.repo_name,source_repo_key:source.repo_key,source_repo_root:source.repo_root,source_cwd:source.source_checkout_path}))});
...
const pane=state.panes.find(x=>x.pane_id===state.pane);state.terminalId=pane&&pane.terminal_id;state.termCols=null;state.termRows=null;state.layoutCols=null;state.layoutRows=null;state.layoutPaneCount=0;
if(state.pane){try{const l=await api('/api/pane-layout?pane_id='+encodeURIComponent(state.pane));if(seq!==refreshSeq)return;const layout=(l.result||{}).layout||{},lp=layout.panes||[];const selected=lp.find(x=>x.pane_id===state.pane);if(selected&&selected.rect){state.termCols=Math.max(1,selected.rect.width);state.termRows=Math.max(1,selected.rect.height);state.layoutCols=Math.max(1,(layout.area||{}).width||state.termCols);state.layoutRows=Math.max(1,(layout.area||{}).height||state.termRows);state.layoutPaneCount=lp.length}}catch(e){}}
if(state.fitDefault||options.fitToBrowser||shouldFitFocusedWebTerminal()||shouldAutoFitDetachedTerminal()){const fit=browserTerminalSize();if(fit){state.termCols=fit.cols;state.termRows=fit.rows;state.fitDefault=false}}
...
function render(){const wsById=Object.fromEntries(state.workspaces.map(w=>[w.workspace_id,w]));const tabById=Object.fromEntries(state.allTabs.concat(state.tabs).map(t=>[t.tab_id,t]));const workspacesHtml=renderSpaces();if(workspacesHtml!==lastWorkspacesHtml){workspaces.innerHTML=workspacesHtml;lastWorkspacesHtml=workspacesHtml}const agentsHtml=renderAgents(wsById,tabById);if(agentsHtml!==lastAgentsHtml){agents.innerHTML=agentsHtml;lastAgentsHtml=agentsHtml}const pane=state.panes.find(p=>p.pane_id===state.pane);const themeIcon=themeMode==='auto'?'A':(themeMode==='dark'?'☾':'☀');const sessionLabel=escapeHtml(state.session||'default');const tabsTools=`<div class="tabs-tools"><button class="mini" id="sessionButtonTabs" title="Session manager" onclick="showSessionManager(state.backendOnline?'Session manager':'Herdr session offline')">${sessionLabel}</button><button class="mini" id="themeToggleTabs" title="Toggle theme" onclick="themeMode=themeMode==='auto'?'dark':(themeMode==='dark'?'light':'auto');applyTheme();render()">${themeIcon}</button><button class="mini" title="Shortcuts" onclick="applyOptions();el('shortcutsModal').style.display='grid'">?</button><button class="mini" title="Settings" onclick="el('settingsModal').style.display='grid';applyOptions()">⚙</button></div>`;const tabsHtml=state.tabs.map(t=>renderTabButton(t)).join('')+(state.ws?`<button class="tab add" title="New panel" onclick="newTab()">+</button>`:'')+tabsTools;if(tabsHtml!==lastTabsHtml){tabs.innerHTML=tabsHtml;lastTabsHtml=tabsHtml}updateTitle(wsById,tabById,pane);if(state.editingTab){const input=document.querySelector('.tab-rename-input');if(input&&document.activeElement!==input){input.focus();input.select()}}if(state.editingWorkspace){const input=document.querySelector('.workspace-rename-input');if(input&&document.activeElement!==input){input.focus();input.select()}}fitTerminalShell()}
...
function tabHoverInfo(t){const panes=state.panes.filter(p=>p.tab_id===t.tab_id);const pane=panes.find(p=>p.pane_id===state.pane)||panes[0];if(!pane)return tabTitle(t);const size=t.tab_id===state.tab&&state.termCols&&state.termRows?` · ${state.termCols}x${state.termRows}`:'';return `${tabTitle(t)} · ${pane.pane_id} · ${pane.terminal_id}${size}`}
...
function renderSpaces(){const groups=new Map(),usedParents=new Set(),linkedIds=new Set();for(const w of state.workspaces){if(!isLinkedWorktree(w))continue;const k=worktreeGroupKey(w);linkedIds.add(w.workspace_id);if(!groups.has(k))groups.set(k,{type:'group',key:k,label:w.worktree.repo_name||w.label,children:[],parent:null});groups.get(k).children.push(w)}for(const g of groups.values()){g.parent=findWorktreeParent(g);if(g.parent)usedParents.add(g.parent.workspace_id)}let items=[];for(const w of state.workspaces){if(linkedIds.has(w.workspace_id)||usedParents.has(w.workspace_id))continue;items.push({type:'single',workspace:w})}for(const g of groups.values())items.push(g);items=sortWorkspaceItems(items);let html='';for(const item of items){if(item.type==='single'){html+=renderWorkspaceCard(item.workspace,'');continue}const children=sortGroupChildren(item.children);html+=renderRepoHeader(item);if(item.parent)html+=renderWorkspaceCard(item.parent,'workspace-group-main');html+=children.map((w,i)=>renderWorkspaceCard(w,'workspace-child '+(i===children.length-1?'last':''))).join('')}return html}
function renderRepoHeader(group){const actions=[],keyToken=encodeURIComponent(group.key);actions.push(group.parent?`<span class="mini tree" title="Create worktree" onclick="event.preventDefault();event.stopPropagation();openWorktreeCreateModal('${group.parent.workspace_id}')">♧+</span>`:`<span class="mini tree" title="Create worktree" onclick="event.preventDefault();event.stopPropagation();openWorktreesForRepo('${keyToken}')">♧+</span>`);if(repoHasUnopenedWorktrees(group))actions.push(`<span class="mini tree" title="Open worktree" onclick="event.preventDefault();event.stopPropagation();openWorktreesForRepo('${keyToken}')">↗</span>`);return `<div class="repo-header workspace-orphan-header ${actions.length?'with-actions':''}"><span>${escapeHtml(group.label)}</span>${actions.length?`<span class="repo-actions">${actions.join('')}</span>`:''}</div>`}
...
function worktreeRowGroupKey(w){return w&&(w.source_repo_key||w.source_repo_root||w.source_repo_name)||''}
function repoRowsForGroup(group){return state.worktrees.filter(w=>w.is_linked_worktree&&worktreeRowGroupKey(w)===group.key)}
function repoHasUnopenedWorktrees(group){return repoRowsForGroup(group).some(w=>!w.open_workspace_id)}
...
function agentToken(cls,value){const s=String(value||'');return `<span class="agent-token ${cls}" title="${escapeAttr(s)}">${escapeHtml(s)}</span>`}
function renderAgentRow(a,wsById,tabById){const w=wsById[a.workspace_id];const repo=w&&w.worktree?parentWorkspaceName(w,wsById):null;const worktree=w&&w.worktree?worktreeDisplayName(w):(w?w.label:a.workspace_id);const t=tabById[a.tab_id];const tab=agentTabLabel(a.workspace_id,t);const fullTitle=(repo?`${repo} › ${worktree}`:worktree)+(tab?` › ${tab}`:'');const titleParts=repo?[agentToken('agent-repo',repo),`<span class="agent-sep">›</span>`,agentToken('agent-worktree',worktree)]:[agentToken('agent-worktree',worktree)];if(tab)titleParts.push(`<span class="agent-sep">›</span>${agentToken('agent-panel',tab)}`);const label=a.name||a.display_agent||a.agent||a.terminal_id;const status=statusClass(a.agent_status);const active=a.workspace_id===state.ws&&a.tab_id===state.tab&&a.pane_id===state.pane;return `<a class="item ${active?'active':''}" title="${escapeAttr(fullTitle)}" href="${escapeAttr(selectionPath(a.workspace_id,a.tab_id,a.pane_id))}" target="herdr-selection" onclick="return navigateSelection(event,'${a.workspace_id}','${a.tab_id}','${a.pane_id}')"><div class="agent-title">${statusMark(a.agent_status,status==='blocked')}${titleParts.join('')}</div><div class="agent-meta"><span class="agent-status ${status}">${escapeHtml(status)}</span><span>•</span><span class="agent-name">${escapeHtml(label)}</span></div></a>`}
...
function pathBasename(path){const parts=String(path||'').replace(/\/+$/,'').split('/').filter(Boolean);return parts.length?parts[parts.length-1]:''}
function worktreeDisplayName(w){if(!w)return 'worktree';const wt=worktreeForWorkspace(w);return pathBasename((wt&&wt.path)||(w.worktree&&w.worktree.checkout_path))||((wt&&wt.label)||w.label)}
...
if(!term){
term=new Terminal({convertEol:false,fontFamily:'ui-monospace,SFMono-Regular,Menlo,monospace',theme:terminalTheme(),scrollback:10000});term.open(terminal);applyTheme();term.onData(sendInputData);
if(term.attachCustomKeyEventHandler)term.attachCustomKeyEventHandler(e=>{if(e.type==='keydown'&&handleCloseShortcut(e))return false;if(options.shiftEnterNewline!==false&&e.type==='keydown'&&e.key==='Enter'&&e.shiftKey&&!e.altKey&&!e.ctrlKey&&!e.metaKey){pasteToTerminal(shiftEnterSequence());return false}if(e.type==='keydown'&&!e.altKey&&!e.ctrlKey&&!e.metaKey&&(e.key==='PageUp'||e.key==='PageDown')){sendBackendScroll(e.key==='PageUp'?'up':'down',Math.max(1,(state.termRows||rows)-1));return false}return true})
}
if(!termScrollBound){
...
el('terminalShell').addEventListener('keydown',e=>{if(options.shiftEnterNewline!==false&&e.shiftKey&&!e.altKey&&!e.ctrlKey&&!e.metaKey&&(e.key==='Enter'||e.code==='Enter'||e.keyCode===13)){e.preventDefault();e.stopImmediatePropagation();sendInputData(shiftEnterSequence())}},true);
...
function modalOpen(){return ['settingsModal','worktreeCreateModal','worktreeOpenModal','shortcutsModal'].some(id=>{const m=el(id);return m&&m.style.display&&m.style.display!=='none'})}
function focusTerminal(){if(state.editingTab||state.editingWorkspace||modalOpen()||!term)return;try{term.focus()}catch(e){}}
function scheduleTerminalFrameWork(){if(Date.now()<pasteFrameUntil)return;if(terminalFramePending)return;terminalFramePending=true;requestAnimationFrame(()=>{terminalFramePending=false;fitTerminalShell();fitTerminalSurface();focusTerminal()})}
...
function sendInputData(data){if(!termWs||termWs.readyState!==1||!data)return;const bytes=inputEncoder.encode(data);const chunkSize=2048;if(bytes.length<=chunkSize&&inputQueue.length===0&&termWs.bufferedAmount<65536){termWs.send(bytes);return}for(let i=0;i<bytes.length;i+=chunkSize)inputQueue.push(bytes.slice(i,i+chunkSize));scheduleInputFlush()}
function scheduleInputFlush(){if(inputFlushTimer)return;inputFlushTimer=setTimeout(flushInputQueue,4)}
function flushInputQueue(){inputFlushTimer=null;if(!termWs||termWs.readyState!==1){inputQueue=[];return}while(inputQueue.length&&termWs.bufferedAmount<65536)termWs.send(inputQueue.shift());if(inputQueue.length)scheduleInputFlush()}
function pasteToTerminal(text){if(!termWs||termWs.readyState!==1||!text)return;const bytes=inputEncoder.encode(text);pasteFrameUntil=Date.now()+250;if(bytes.length<=32*1024*1024&&inputQueue.length===0&&termWs.bufferedAmount<1024*1024){termWs.send(bytes);setTimeout(()=>{pasteFrameUntil=0;scheduleTerminalFrameWork()},250);return}sendInputData(text);setTimeout(()=>{pasteFrameUntil=0;scheduleTerminalFrameWork()},250)}
...
function shouldFitFocusedWebTerminal(){return !document.hidden&&(!document.hasFocus||document.hasFocus())}
function shouldAutoFitDetachedTerminal(){if(options.fitToBrowser)return false;const fit=browserTerminalSize();if(!fit)return false;const singlePane=(state.layoutPaneCount||1)===1;return singlePane&&state.layoutCols===state.termCols&&state.layoutRows===state.termRows&&(fit.cols>state.termCols||fit.rows>state.termRows)}
function fitFocusedTerminal(){if(!state.terminalId||!shouldFitFocusedWebTerminal())return;const fit=browserTerminalSize();if(!fit)return;state.termCols=fit.cols;state.termRows=fit.rows;connectTerminal()}
window.addEventListener('resize',()=>{if(resizeFramePending)return;resizeFramePending=true;requestAnimationFrame(()=>{resizeFramePending=false;fitTerminalShell();if(options.fitToBrowser||shouldFitFocusedWebTerminal()||shouldAutoFitDetachedTerminal()){const fit=browserTerminalSize();if(fit){state.termCols=fit.cols;state.termRows=fit.rows;connectTerminal()}}})});
window.addEventListener('focus',()=>requestAnimationFrame(fitFocusedTerminal));
...
function textValue(v){if(v===null||v===undefined)return '';if(typeof v==='string')return v;if(typeof v==='number'||typeof v==='boolean')return String(v);if(v.path)return textValue(v.path);if(v.display)return textValue(v.display);if(v.label)return textValue(v.label);if(v.name)return textValue(v.name);return ''}
...
function setWorktreeLoading(show){const loading=el('worktreeLoading');if(loading)loading.classList.toggle('show',!!show)}
function closeWorktreeOpenModal(){clearTimeout(state.openWorktreeAutodiscoverTimer);setWorktreeLoading(false);const m=el('worktreeOpenModal');if(m)m.style.display='none'}
function openWorktreeOpenModal(){state.openWorktreeSelected=null;state.openWorktreePathSuggestions=[];state.openWorktreeSource=null;state.openWorktreeRows=[];state.openWorktreeAllRows=[];state.openWorktreeBranches=[];el('worktreeNewBranch').value='';el('worktreeNewBase').value='';el('worktreeNewLabel').value='';el('worktreeNewPath').value='';syncWorktreeBranchOptions([]);renderWorktreeOpenList();el('worktreeOpenError').textContent='';el('worktreeOpenModal').style.display='grid';setTimeout(()=>{el('worktreeDiscoverPath').focus();loadWorktreePathSuggestions()},0)}
function openWorktreesForRepo(keyToken){const key=decodeURIComponent(keyToken),allRows=state.worktrees.filter(w=>worktreeRowGroupKey(w)===key),rows=allRows.filter(w=>w.is_linked_worktree);const source=allRows[0]||rows[0]||{};state.openWorktreeSelected=null;state.openWorktreePathSuggestions=[];state.openWorktreeSource={workspace_id:source.source_workspace_id||null,cwd:source.source_cwd||null,repo_name:source.source_repo_name||'',repo_root:source.source_repo_root||''};state.openWorktreeAllRows=allRows.map(w=>Object.assign({},w,{path:textValue(w.path),label:textValue(w.label),branch:textValue(w.branch),source_cwd:textValue(w.source_cwd),source_repo_root:textValue(w.source_repo_root)}));state.openWorktreeRows=rows.map(w=>Object.assign({},w,{path:textValue(w.path),label:textValue(w.label),branch:textValue(w.branch),source_cwd:textValue(w.source_cwd),source_repo_root:textValue(w.source_repo_root)}));el('worktreeNewBranch').value='';el('worktreeNewBase').value='';el('worktreeNewLabel').value='';el('worktreeNewPath').value='';syncWorktreeBranchOptions([]);el('worktreeOpenError').textContent='';el('worktreeDiscoverPath').value=source.source_cwd||source.source_repo_root||'';renderWorktreeOpenList();el('worktreeOpenModal').style.display='grid';loadWorktreeBranchOptions()}
function validOpenWorktreeRows(){return (state.openWorktreeRows||state.worktrees||[]).map(w=>Object.assign({},w,{path:textValue(w.path),label:textValue(w.label),branch:textValue(w.branch),source_cwd:textValue(w.source_cwd)})).filter(w=>w.is_linked_worktree&&(w.source_workspace_id||w.source_cwd))}
function syncWorktreePathOptions(rows){const optionsEl=el('worktreePathOptions');if(!optionsEl)return;const seen=new Set(),items=[];for(const w of rows||[]){if(!w.path||seen.has(w.path))continue;seen.add(w.path);items.push(`<option value="${escapeAttr(w.path)}">${escapeAttr(w.label||w.branch||'worktree')}</option>`)}for(const s of state.openWorktreePathSuggestions||[]){if(!s.path||seen.has(s.path))continue;seen.add(s.path);items.push(`<option value="${escapeAttr(s.path)}">${escapeAttr(s.label||'directory')}</option>`)}optionsEl.innerHTML=items.join('')}
function scheduleWorktreePathSuggestions(){clearTimeout(state.openWorktreePathSuggestTimer);state.openWorktreePathSuggestTimer=setTimeout(loadWorktreePathSuggestions,120)}
async function loadWorktreePathSuggestions(){const input=el('worktreeDiscoverPath');if(!input)return;const prefix=input.value;try{const r=await api('/api/path-suggestions?prefix='+encodeURIComponent(prefix));if(input.value!==prefix)return;state.openWorktreePathSuggestions=r.suggestions||[];syncWorktreePathOptions(validOpenWorktreeRows())}catch(_){state.openWorktreePathSuggestions=[];syncWorktreePathOptions(validOpenWorktreeRows())}}
function syncWorktreeBranchOptions(branches){const optionsEl=el('worktreeBranchOptions');if(!optionsEl)return;optionsEl.innerHTML=(branches||[]).map(branch=>`<option value="${escapeAttr(branch)}"></option>`).join('')}
async function loadWorktreeBranchOptions(){const source=state.openWorktreeSource;if(!source||!source.cwd){state.openWorktreeBranches=[];syncWorktreeBranchOptions([]);return}try{const r=await api('/api/git-branches?cwd='+encodeURIComponent(source.cwd));state.openWorktreeBranches=r.branches||[];syncWorktreeBranchOptions(state.openWorktreeBranches)}catch(_){state.openWorktreeBranches=[];syncWorktreeBranchOptions([])}}
function branchExistsForNewWorktree(branch){branch=String(branch||'').trim();if(!branch)return false;return (state.openWorktreeBranches||[]).includes(branch)||(state.openWorktreeRows||[]).some(w=>textValue(w.branch)===branch)}
function checkedOutWorktreeForBranch(branch){branch=String(branch||'').trim();if(!branch)return null;return (state.openWorktreeAllRows||state.openWorktreeRows||[]).find(w=>textValue(w.branch)===branch&&!w.is_prunable)||null}
function defaultBaseBranch(){const branches=state.openWorktreeBranches||[];if(branches.includes('main'))return 'main';if(branches.includes('master'))return 'master';return branches[0]||''}
function syncBranchNameFromBase(){const base=el('worktreeNewBase').value.trim(),branchInput=el('worktreeNewBranch');if(!base||base===defaultBaseBranch()||branchInput.value.trim())return;if(branchExistsForNewWorktree(base))return;branchInput.value=base}
function firstWorktreePathSuggestion(){const rows=validOpenWorktreeRows();if(rows.length)return rows[0].path;const suggestions=state.openWorktreePathSuggestions||[];return suggestions.length?textValue(suggestions[0].path):''}
function acceptFirstWorktreePathSuggestion(){const path=firstWorktreePathSuggestion();if(!path)return false;el('worktreeDiscoverPath').value=path;const idx=(state.openWorktreeRows||[]).findIndex(w=>textValue(w.path)===path&&w.is_linked_worktree);state.openWorktreeSelected=idx>=0?idx:null;renderWorktreeOpenList();scheduleWorktreeAutodiscover();return true}
function scheduleWorktreeAutodiscover(){clearTimeout(state.openWorktreeAutodiscoverTimer);const seconds=Number(options.worktreeAutoDiscoverSeconds)||0;const value=el('worktreeDiscoverPath').value.trim();setWorktreeLoading(false);if(!value)return;setWorktreeLoading(true);state.openWorktreeAutodiscoverTimer=setTimeout(()=>{if(el('worktreeOpenModal').style.display==='grid'&&el('worktreeDiscoverPath').value.trim()===value)discoverWorktrees(true);else setWorktreeLoading(false)},Math.max(0,seconds)*1000)}
function updateWorktreeNewVisibility(){const section=el('worktreeNewSection');if(section)section.style.display=state.openWorktreeSource?'block':'none'}
function renderWorktreeOpenList(){const list=el('worktreeOpenList');if(!list)return;const rows=validOpenWorktreeRows();state.openWorktreeRows=rows;syncWorktreePathOptions(rows);updateWorktreeNewVisibility();if(!rows.length){list.innerHTML=`<div class="worktree-open-empty">${state.openWorktreeSource?'No linked worktrees found. Create a new one below.':'Enter a repo path or worktrees folder. Discovery starts automatically.'}</div>`;return}list.innerHTML=rows.map((w,i)=>`<div class="worktree-open-row ${state.openWorktreeSelected===i?'selected':''}"><span><strong>${escapeHtml(w.label||w.branch||'worktree')}</strong><small>${escapeHtml(w.branch||'detached')} · ${escapeHtml(w.path)}</small></span><span class="session-controls"><button class="mini danger" title="Remove worktree from disk" onclick="event.stopPropagation();removeDiscoveredWorktree(${i})">🗑</button><button class="btn" onclick="openDiscoveredWorktree(${i})">Open</button></span></div>`).join('')}
async function discoverWorktrees(){clearTimeout(state.openWorktreeAutodiscoverTimer);setWorktreeLoading(true);const err=el('worktreeOpenError'),path=el('worktreeDiscoverPath').value.trim();err.textContent='';state.openWorktreeSelected=null;try{let url='/api/worktrees';if(path)url+='?cwd='+encodeURIComponent(path);const r=await api(url);const source=(r.result||{}).source||{};const sourceCwd=textValue(source.source_checkout_path)||path||null;state.openWorktreeSource={workspace_id:source.source_workspace_id||null,cwd:sourceCwd,repo_name:textValue(source.repo_name),repo_root:textValue(source.repo_root)};state.openWorktreeAllRows=((r.result||{}).worktrees||[]).map(w=>Object.assign({},w,{path:textValue(w.path),label:textValue(w.label),branch:textValue(w.branch),source_workspace_id:source.source_workspace_id,source_repo_name:textValue(source.repo_name),source_repo_root:textValue(source.repo_root),source_cwd:sourceCwd}));state.openWorktreeRows=state.openWorktreeAllRows;renderWorktreeOpenList();loadWorktreeBranchOptions()}catch(ex){state.openWorktreeSource=null;state.openWorktreeRows=[];state.openWorktreeAllRows=[];syncWorktreeBranchOptions([]);renderWorktreeOpenList();err.textContent=ex.message||String(ex)}finally{setWorktreeLoading(false)}}
async function openDiscoveredWorktree(index){const row=(state.openWorktreeRows||[])[index];if(!row||!row.is_linked_worktree)return;const err=el('worktreeOpenError');err.textContent='';try{const r=await api('/api/worktrees/open',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({workspace_id:row.source_workspace_id||null,cwd:row.source_workspace_id?null:row.source_cwd,path:row.path,label:null})});closeWorktreeOpenModal();const result=r.result||{};go(result.workspace.workspace_id,result.tab&&result.tab.tab_id,result.root_pane&&result.root_pane.pane_id)}catch(ex){err.textContent=ex.message||String(ex)}}
async function removeDiscoveredWorktree(index){const row=(state.openWorktreeRows||[])[index];if(!row||!row.is_linked_worktree)return;if(!confirm(`Remove worktree "${row.label||row.branch||row.path}" from disk?`))return;const err=el('worktreeOpenError');err.textContent='';try{if(row.open_workspace_id)await api(`/api/workspaces/${encodeURIComponent(row.open_workspace_id)}/worktree-remove`,{method:'POST'});else await api('/api/worktrees/remove-path',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({repo_root:row.source_repo_root,path:row.path,force:false})});await discoverWorktrees();refresh()}catch(ex){err.textContent=ex.message||String(ex)}}
async function createDiscoveredWorktree(){const err=el('worktreeOpenError'),sourcePath=el('worktreeDiscoverPath').value.trim(),branch=el('worktreeNewBranch').value.trim(),base=el('worktreeNewBase').value.trim(),label=el('worktreeNewLabel').value.trim(),path=el('worktreeNewPath').value.trim(),submit=el('worktreeNewSubmit');err.textContent='';if(branch&&base&&branch===base){err.textContent='Branch name must differ from base branch';return}submit.disabled=true;try{if(sourcePath&&!state.openWorktreeSource)await discoverWorktrees();const checkedOut=checkedOutWorktreeForBranch(branch);if(checkedOut){err.textContent=`Branch "${branch}" is already checked out at ${textValue(checkedOut.path)}`;return}if(branchExistsForNewWorktree(branch)){err.textContent=`Branch "${branch}" already exists`;return}const source=state.openWorktreeSource||{};const workspaceId=source.workspace_id||(!sourcePath?state.ws:null),cwd=source.workspace_id?null:(source.cwd||sourcePath||null);const r=await api('/api/worktrees',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({workspace_id:workspaceId,cwd,branch:branch||null,base:base||null,label:label||null,path:path||null})});closeWorktreeOpenModal();const result=r.result||{};go(result.workspace.workspace_id,result.tab&&result.tab.tab_id,result.root_pane&&result.root_pane.pane_id)}catch(ex){err.textContent=ex.message||String(ex)}finally{submit.disabled=false}}
async function closeWorkspace(id){const w=state.workspaces.find(x=>x.workspace_id===id),kind=isLinkedWorktree(w)?'worktree':'workspace';if(!confirm(`Close ${kind} "${workspaceCloseName(id)}"?`))return;await api(`/api/workspaces/${encodeURIComponent(id)}/close`,{method:'POST'});if(state.ws===id){state.ws=null;state.tab=null;state.pane=null}refresh()}
...
async function newTab(){if(!state.ws)return;const r=await api('/api/tabs',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({workspace_id:state.ws})});const tab=r.result.tab.tab_id;go(state.ws,tab)}
...
let settingsBackdropDown=false,shortcutsBackdropDown=false;el('settingsToggle').onclick=()=>{el('settingsModal').style.display='grid';applyOptions()};el('settingsClose').onclick=()=>{el('settingsModal').style.display='none'};el('settingsModal').addEventListener('pointerdown',e=>{settingsBackdropDown=e.target===el('settingsModal')});el('settingsModal').addEventListener('click',e=>{if(settingsBackdropDown&&e.target===el('settingsModal'))el('settingsModal').style.display='none';settingsBackdropDown=false});el('shortcutsToggle').onclick=()=>{applyOptions();el('shortcutsModal').style.display='grid'};el('shortcutsClose').onclick=()=>{el('shortcutsModal').style.display='none'};el('shortcutsCloseTop').onclick=()=>{el('shortcutsModal').style.display='none'};el('shortcutsModal').addEventListener('pointerdown',e=>{shortcutsBackdropDown=e.target===el('shortcutsModal')});el('shortcutsModal').addEventListener('click',e=>{if(shortcutsBackdropDown&&e.target===el('shortcutsModal'))el('shortcutsModal').style.display='none';shortcutsBackdropDown=false});el('optTheme').onchange=()=>{themeMode=normalizeThemeMode(el('optTheme').value);applyTheme()};el('optOverflow').onchange=()=>{options.overflow=el('optOverflow').checked;saveOptions();applyOptions()};el('optFit').onchange=()=>{options.fitToBrowser=el('optFit').checked;saveOptions();applyOptions()};el('optShiftEnterNewline').onchange=()=>{options.shiftEnterNewline=el('optShiftEnterNewline').checked;saveOptions();applyOptions()};el('optCloseShortcut').oninput=saveCloseShortcutOption;el('optCloseShortcut').onchange=saveCloseShortcutOption;el('optSortAgents').onchange=()=>{options.sortAgentsByStatus=el('optSortAgents').checked;saveOptions();applyOptions();render()};el('optWorkspaceSort').onchange=()=>{options.workspaceSort=el('optWorkspaceSort').value;saveOptions();applyOptions();render()};el('optSoundScope').onchange=()=>{options.soundScope=el('optSoundScope').value;saveOptions();applyOptions()};el('optScrollLines').oninput=()=>{options.scrollLines=Math.max(1,Math.min(20,Number(el('optScrollLines').value)||3));saveOptions();applyOptions()};el('optWorktreeAutoDiscover').oninput=()=>{options.worktreeAutoDiscoverSeconds=Math.max(0,Math.min(30,Number(el('optWorktreeAutoDiscover').value)||0));saveOptions();applyOptions();scheduleWorktreeAutodiscover()};el('optSound').onchange=()=>{options.sound=el('optSound').checked;saveOptions();applyOptions()};
el('worktreeCreateClose').onclick=closeWorktreeCreateModal;el('worktreeCreateCancel').onclick=closeWorktreeCreateModal;el('worktreeCreateModal').addEventListener('click',e=>{if(e.target===el('worktreeCreateModal'))closeWorktreeCreateModal()});el('worktreeCreateForm').onsubmit=async e=>{e.preventDefault();const err=el('worktreeCreateError'),submit=el('worktreeCreateSubmit');err.textContent='';const branch=el('worktreeBranch').value.trim(),base=el('worktreeBase').value.trim(),label=el('worktreeLabel').value.trim(),path=el('worktreePath').value.trim();submit.disabled=true;try{const r=await api('/api/worktrees',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({workspace_id:state.createWorktreeWorkspace,branch:branch||null,base:base||null,label:label||null,path:path||null})});closeWorktreeCreateModal();const result=r.result||{};go(result.workspace.workspace_id,result.tab&&result.tab.tab_id,result.root_pane&&result.root_pane.pane_id)}catch(ex){err.textContent=ex.message||String(ex)}finally{submit.disabled=false}};
el('worktreeOpenClose').onclick=closeWorktreeOpenModal;el('worktreeOpenModal').addEventListener('click',e=>{if(e.target===el('worktreeOpenModal'))closeWorktreeOpenModal()});el('worktreeOpenRefresh').onclick=async()=>{await refresh();state.openWorktreeRows=[];state.openWorktreeAllRows=[];state.openWorktreeSelected=null;state.openWorktreeSource=null;state.openWorktreeBranches=[];syncWorktreeBranchOptions([]);renderWorktreeOpenList();scheduleWorktreeAutodiscover()};el('worktreeNewForm').onsubmit=e=>{e.preventDefault();createDiscoveredWorktree()};el('worktreeNewBase').addEventListener('input',syncBranchNameFromBase);el('worktreeNewBase').addEventListener('change',syncBranchNameFromBase);function worktreePathInputChanged(){const value=el('worktreeDiscoverPath').value.trim();const idx=(state.openWorktreeRows||[]).findIndex(w=>textValue(w.path)===value&&w.is_linked_worktree);state.openWorktreeSelected=idx>=0?idx:null;state.openWorktreeSource=null;state.openWorktreeRows=[];state.openWorktreeAllRows=[];state.openWorktreeBranches=[];syncWorktreeBranchOptions([]);scheduleWorktreePathSuggestions();scheduleWorktreeAutodiscover();renderWorktreeOpenList()}el('worktreeDiscoverPath').addEventListener('input',worktreePathInputChanged);el('worktreeDiscoverPath').addEventListener('change',worktreePathInputChanged);el('worktreeDiscoverPath').addEventListener('keydown',e=>{if(e.key==='Tab'&&acceptFirstWorktreePathSuggestion()){e.preventDefault();e.stopPropagation()}});
function editableEventTarget(e){const t=e.target;return t&&(t.isContentEditable||['INPUT','TEXTAREA','SELECT'].includes(t.tagName))}
el('copyMenu').onclick=copySelection;el('pasteMenu').onclick=pasteClipboard;el('terminalShell').addEventListener('contextmenu',e=>{e.preventDefault();showClipboardMenu(e.clientX,e.clientY)});document.addEventListener('click',e=>{const menu=el('clipboardMenu');if(menu&&!menu.contains(e.target))hideClipboardMenu()});window.addEventListener('keydown',closeShortcutKeydown,true);document.addEventListener('keydown',e=>{if(editableEventTarget(e))return;const copyKey=(e.metaKey||e.ctrlKey)&&!e.shiftKey&&!e.altKey&&e.key.toLowerCase()==='c';const pasteKey=(e.metaKey||e.ctrlKey)&&!e.shiftKey&&!e.altKey&&e.key.toLowerCase()==='v';if(copyKey&&term&&term.getSelection&&term.getSelection()){e.preventDefault();copySelection()}else if(pasteKey){e.preventDefault();pasteClipboard()}});
